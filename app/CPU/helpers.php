...
class Helpers
{
    ...
    public static function currency_converter($amount)
    {
        ...
        if ($currency_model == 'multi_currency') {
            if (session()->has('Rupiah')) {
                $Rupiah = session('Rupiah');
            } else {
                $Rupiah = Currency::where(['code' => 'Rupiah'])->first()->exchange_rate;
                session()->put('Rupiah', $Rupiah);
            }
            $my_currency = \session('currency_exchange_rate');
            $rate = $my_currency / $Rupiah;
        } else {
            $rate = 1;
        }
    ...
    public static function convert_currency_to_usd($price)
    {
        ...
        if ($currency_model == 'multi_currency') {
            ...
            $code = session('currency_code') == null ? 'Rupiah' : session('currency_code');
